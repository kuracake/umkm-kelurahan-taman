...
export function ProdukList({ produks }: { produks: ProdukWithRelations[] }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProdukWithRelations | null>(
    null
  );
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleting(true);
    await deleteProdukAction(deleteTarget.id);
    setDeleting(false);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `"${deleteTarget.namaProduk}" akan dihapus permanen dan tidak bisa dikembalikan.`
            : ""
        }
